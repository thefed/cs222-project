...
RC RM_TEST_PRIVATE_EXTRA_1(const string &tableName)
{
    ...
    // 2. Read Attributes																																																				
    // 3. Drop Attributes **
    cerr << endl << "***** In RM Private Extra Credit Test Case 1 *****" << endl;
    ...
    {
        cerr << "***** RelationManager::readAttribute() failed. Private Extra Credit Test Case 1 Failed. *****" << endl;
        cerr << endl;
        free(returnedData);
        free(tuple);
        return -1;
    ...
    else
    {
		...
		// Print the tuple
		cerr << "Before dropping an attribute:" << endl;
        ...
        if (newAttrs.size() != attrs.size() - 1) {
			cerr << "The number of attributes after dropping an attribute is not correct." << endl;
			cerr << "***** [FAIL] Private Extra Credit Test Case 1 Failed. *****" << endl << endl;
			free(returnedData);
			free(tuple);
			return -1;
        }
		cerr << endl << "After dropping an attribute:" << endl;
    ...
    if(rc != success) {
    	cerr << "***** A scan initialization should not fail. *****" << endl;
        cerr << "***** [FAIL] Private Extra Credit Test Case 1 Failed. *****" << endl << endl;
		free(tuple);
		free(returnedData);
        return -1;
		...
		while(rmsi.getNextTuple(rid, returnedData) != RM_EOF)
		{
			counter++;
			if (counter > 2) {
				cerr << "***** A wrong entry was returned. RM Private Extra Credit Test Case 1 failed *****" << endl << endl;
				rmsi.close();
				free(tuple);
				free(returnedData);
				return -1;
			...
			cerr << "Returned message_text: " << strReturned << endl;
		}
	} 
	...
	if (counter != 2) {
    	cerr << "***** A scan result is not correct. *****" << endl;
		cerr << "***** [FAIL] Private Extra Credit Test Case 1 Failed. *****" << endl << endl;
		free(tuple);
		free(returnedData);
		return -1;
    ...
    cerr << "***** Private Extra Credit Test Case 1 finished. The result will be examined. *****" << endl;
    return success;
}
