// this file is copied from project 4, modified to run the test cases of project 2
#include "rm.h"
...
RC RelationManager::createCatalog()
{
	...
	rc = rbf->createFile(TABLES);
	assert(rc == 0);
	rc = rbf->openFile(TABLES, fileHandle);
	assert(rc == 0);
	...
	rc = rbf->insertRecord(fileHandle, tablesDescriptor, tablesInfo, rid);
	assert(rc == 0);
	...
	// Create file "Columns"
	rc = rbf->createFile(COLUMNS);
	assert(rc == 0);
	rc = rbf->openFile(COLUMNS, fileHandle);
	assert(rc == 0);
	...
	rc = rbf->insertRecord(fileHandle, columnsDescriptor, columnsInfo, rid);
	assert(rc == 0);
...
RC RelationManager::deleteCatalog()
{
	RC rc;
	RecordBasedFileManager *rbf = RecordBasedFileManager::instance();
	rc = rbf->destroyFile(COLUMNS);
	assert(rc == 0);
	rc = rbf->destroyFile(TABLES);
	assert(rc == 0);
	return 0;
}
...
RC RelationManager::createTable(const string &tableName, const vector<Attribute> &attrs)
{
	...
	{
		rc = rbfm_ScanIterator.close(); // If table not found, close Tables
		assert(rc == 0);
		free(returnedData);
...
RC RelationManager::getAttributes(const string &tableName, vector<Attribute> &attrs)
{
	...
	{
		// printf("getAttributes() of table '%s'\n", tableName.c_str());
...
RC RelationManager::readTuple(const string &tableName, const RID &rid, void *data)
{
	...
	RC rc2 = rbf->closeFile(fileHandle);
	assert(rc2 == 0);
	return rc;
}
...
// comment out the functions to be implemented in project 3
/*
