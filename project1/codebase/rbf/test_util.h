...
ifstream::pos_type getFileSize(string fileName)
{
    ...
    double pageCount = ((double) in.tellg()) / (double) PAGE_SIZE;
    cout << endl << fileName << " - file size:" << in.tellg() << " page count:" << pageCount << endl;
    return in.tellg();
}
...
bool compareFileSizes(string fileName1, string fileName2) {
	...
	s1 = in1.tellg();
    double pageCount1 = ((double) s1) / (double) PAGE_SIZE;

	ifstream in2(fileName2.c_str(), ifstream::in | ifstream::binary);
	in2.seekg(0, ifstream::end);
	s2 = in2.tellg();
    double pageCount2 = ((double) s2) / (double) PAGE_SIZE;

	cout << endl << fileName1 << " size: " << s1 << "  page count: " << pageCount1 << " // " << fileName2 << " size: " << s2 << " page count: " << pageCount2 << endl;
