...
RC RecordBasedFileManager::deleteRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const RID &rid)
{
    ...
    if (rid.slotNum < (unsigned) slotCount - 1) // This is not the last record in page, to move its following records is needed!
    {
        ...
        memmove((char *) curPageData + delRecordOffset, (char *) curPageData + delRecordOffset + delRecordLength,
...
RC RecordBasedFileManager::updateRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, const RID &rid)
{
    ...
    {
        // 1. move following records
        memmove((char *) curPageData + oldRecordOffset + updatedRecordLength, (char *) curPageData + oldRecordOffset + oldRecordLength,
...
RC readAttrsFromPage(FileHandle &fileHandle, const void *curPageData, const vector<Attribute> &recordDescriptor, const unsigned slotNum,
        const vector<int> returnAttrIndexes, void *data)
{
    ...
    if (returnAttrIndexes.size() == 0) // No attributes given, read failed!
        return -1;
    short recordOffset = *(short *)((char *) curPageData + PAGE_SIZE - (2 * slotNum + 4) * sizeof(short));
    short recordLength = *(short *)((char *) curPageData + PAGE_SIZE - (2 * slotNum + 3) * sizeof(short));
    ...
    {
        unsigned pageNum = *(unsigned *)((char *) curPageData + recordOffset);
        unsigned newSlotNum = *(unsigned *)((char *) curPageData + recordOffset + sizeof(unsigned));
        void *newPageData = malloc(PAGE_SIZE);
        fileHandle.readPage(pageNum, newPageData);
        RC rc = readAttrsFromPage(fileHandle, newPageData, recordDescriptor, newSlotNum, returnAttrIndexes, data);
        free(newPageData);
        return rc;
    }
...
RC readAttrFromPage(FileHandle &fileHandle, const void *curPageData, const vector<Attribute> &recordDescriptor,
        const unsigned slotNum, const string &attributeName, void *data)
{
    ...
    {
        int pageNum = *(unsigned *)((char *) curPageData + recordOffset);
        int newSlotNum = *(unsigned *)((char *) curPageData + recordOffset + sizeof(unsigned));
        void *newPageData = malloc(PAGE_SIZE);
        fileHandle.readPage(pageNum, newPageData);
        RC rc = readAttrFromPage(fileHandle, curPageData, recordDescriptor, newSlotNum, attributeName, data);
        free(newPageData);
        return rc;
    }
    ...
    unsigned short attrOffset1 = *(unsigned short *)((char *) curPageData + recordOffset + index * sizeof(short));
    unsigned short attrOffset2 = *(unsigned short *)((char *) curPageData + recordOffset + (index + 1) * sizeof(short));
    // Read attribute
    if (!nullValue)
        ...
        const string &attributeName, void *data)
{
    ...
    int index = getAttrIndex(recordDescriptor, attributeName);
    if (index == -1 && attributeName != "") // Given a valid attr name, but not found, read attribute failed!
        return -1;
    short recordOffset;
    short recordLength;
    void *curPageData = malloc(PAGE_SIZE);
        ...
        const void *value, const vector<string> &attributeNames)
{
    ...
    if (fileHandle.getNumberOfPages() > 0)
        fileHandle.readPage(0, curPageData);
...
RC RBFM_ScanIterator::getNextRecord(RID &rid, void *data)
{
    ...
    if (noMatchingRecord) // Compare with NULL compVal should fail!
        return -1;
    short slotCount = *(short *)((char *) curPageData + PAGE_SIZE - 2 * sizeof(short));

    void *conditionValue = malloc(PAGE_SIZE);
    bool isMatchFlag = false;
    while (!isMatchFlag)
    {
        if (slotNum > slotCount - 1) // Read next page, update curPageData, slotCount
        {
            pageNum++;
            slotNum = 0;
            ...
            slotCount = *(short *)((char *) curPageData + PAGE_SIZE - 2 * sizeof(short));
        }
        short recordOffset = *(short *)((char *) curPageData + PAGE_SIZE - (2 * slotNum + 4) * sizeof(short));
        if (recordOffset >= 0) // Record exists
        {
            ...
            {
                int rc = readAttrFromPage(fileHandle, curPageData, recordDescriptor, slotNum, conditionAttr, conditionValue);
            ...
            {
                //				cout << "getNextRecord() Found ";// << recordDescriptor[1].name;
                //				printKey((char *)conditionValue + 1, recordDescriptor[1]);
                rid.pageNum = pageNum;
                rid.slotNum = slotNum;
                int rc = readAttrsFromPage(fileHandle, curPageData, recordDescriptor, slotNum, returnAttrIndexes, data);
