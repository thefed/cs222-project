...
RC RelationManager::createCatalog()
{
	...
	void *tablesInfo = (void *) malloc(TABLE_INFO_SIZE);
	RID rid;
...
RC RelationManager::createTable(const string &tableName, const vector<Attribute> &attrs)
{
	...
	rbf->openFile("Tables", rbfm_ScanIterator.fileHandle); // Open Tables
	vector<Attribute> tablesDescriptor; //
	createTablesDescriptor(tablesDescriptor);
	...
	rbf->scan(rbfm_ScanIterator.fileHandle, tablesDescriptor, condAttr, EQ_OP, value, attributes, rbfm_ScanIterator);
	...
	{
		cout << "Creating ";
		printTuple(tablesDescriptor, tablesInfo);
	}
	rbf->insertRecord(rbfm_ScanIterator.fileHandle, tablesDescriptor, tablesInfo, rid);
...
//				printTuple(columnsDescriptor,columnsInfo);
		rbf->insertRecord(rbfm_ScanIterator.fileHandle, columnsDescriptor, columnsInfo, rid);
	}
	free(columnsInfo);
	rbf->closeFile(rbfm_ScanIterator.fileHandle);
...
{
	bool invalidDeletion = tableName == "" || tableName == "Tables" || tableName == "Columns";
	...
	for (unsigned i = 0; i < attrs.size(); i++)
	{
		string indexFileName = tableName + "_" + attrs[i].name;
	...
	{
		rbf->destroyFile(deleteTableNames[i]); // Remove file first, then clear catalog info
		...
		// Start scan
		RBFM_ScanIterator rbfm_ScanIterator;
		rbf->openFile(TABLES, rbfm_ScanIterator.fileHandle);
		...
		while (rbfm_ScanIterator.getNextRecord(rid, returnedData) != RBFM_EOF)
		{
			rbf->deleteRecord(rbfm_ScanIterator.fileHandle, tablesDescriptor, rid);
			isTableExist = true;
			break;
		}
		rbfm_ScanIterator.close();
		free(value);
		if (!isTableExist) // If no table, then no index of course
		{
			free(returnedData);
		...
		else
		{
			...
			rbf->scan(rbfm_ScanIterator.fileHandle, tablesDescriptor, condAttr, EQ_OP, &deletedTableId, attributesColumn, rbfm_ScanIterator);
			while (rbfm_ScanIterator.getNextRecord(rid, returnedData) != RBFM_EOF)
			{
...
RC RelationManager::getAttributes(const string &tableName, vector<Attribute> &attrs)
{
	...
	else
	{
		...
		rbf->scan(rbfm_ScanIterator.fileHandle, tablesDescriptor, condAttr, EQ_OP, value, attributesTable, rbfm_ScanIterator);
...
//			cout << "RM::getAttributes() failed, table(" << tableName << ") not found in Tables!\n";
		...
		rbf->openFile(COLUMNS, rbfm_ScanIterator2.fileHandle);
		vector<Attribute> columnsDescriptor;
		createColumnsDescriptor(columnsDescriptor);
		...
		rbf->scan(rbfm_ScanIterator2.fileHandle, columnsDescriptor, condAttr, EQ_OP, &tableID, attributesColumn, rbfm_ScanIterator2);
...
RC RelationManager::insertTuple(const string &tableName, const void *data, RID &rid)
{
	...
	if (getAttributes(tableName, tableDescriptor) == -1)
		return -1;
	FileHandle fileHandle;
	RecordBasedFileManager *rbf = RecordBasedFileManager::instance();
	...
	for (unsigned i = 0; i < tableDescriptor.size(); i++)
	{
		string indexFileName = tableName + "_" + tableDescriptor[i].name;
		RBFM_ScanIterator rbfm_ScanIterator;
		...
		if (getAttributes(indexFileName, indexAttrs) == 0) // Index file exists, update!
		{
			...
			readAttribute(tableName, rid, indexAttrs[0].name, key);
			ix->openFile(indexFileName, ixfileHandle);
			//			cout << "Inserting key: ";
			//			printKey((char *)key + 1, indexAttrs[0]);
			ix->insertEntry(ixfileHandle, indexAttrs[0], (char *) key + 1, rid);
...
//	printKey(value, recordDescriptor[0]);
	...
	rbf->scan(rm_ScanIterator.rbfmsi.fileHandle, recordDescriptor, conditionAttribute, compOp, value, attributeNames, rm_ScanIterator.rbfmsi);
...
RC RelationManager::createIndex(const string &tableName, const string &attributeName)
{
	...
	assert(rc == 0 && "RelationManager::scan() should not fail.");
	void *returnedData = malloc(indexAttrs[0].length + 4);
	RID rid;
		...
		bool highKeyInclusive, RM_IndexScanIterator &rm_IndexScanIterator)
{
	...
	{
		cout << "index file(" << indexFileName << ") does not exist!\n";
		return -1;
	}
	IndexManager* ix = IndexManager::instance();
	IXFileHandle ixfileHandle;
	ix->openFile(indexFileName, ixfileHandle);
	int rc = ix->scan(ixfileHandle, attrs[0], lowKey, highKey, lowKeyInclusive, highKeyInclusive, rm_IndexScanIterator.ixsi);
